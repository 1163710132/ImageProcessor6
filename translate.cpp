...
const string host{"translate.google.cn"};
...
string cal_token(const string & text) {
    ...
    return std::to_string(a) + "." + std::to_string(a ^ t0);
}
...
void get_tkk(ssl::stream<tcp::socket> & stream) {
    ...
    std::regex rgx{"TKK[:=]'(\\d+?).(\\d+?)'", std::regex::icase | std::regex::optimize};
    std::smatch sm;
...
string google_translate_stdstring(string text, string fromLang, string toLang){
    ...
    try {
        ...
        get_tkk(stream);
        //TRANSLATE
        string result = translate(stream, text);
        //CLOSE
